...
```{r}
df_tufe=read_delim("tufe_raw.csv",  delim="|")%>%select(4,5,6)%>%tail(-1)%>%head(-1)%>%rename(b_tufe=Sütunlar, year=X4, month=X5)%>%fill(year)%>%separate(month,c("month_num","month_text"),sep="-")%>%unite("date_merge", year:month_num,sep="-")%>%mutate(date=ym(date_merge))%>%select(-month_text,-date_merge)%>%glimpse()
...
```{r}
df_property=read_excel("property_price_index.xlsx")%>%mutate(date=my(Tarih))%>%filter(is.na(date)==FALSE)%>%rename(b_property_index=`TP HKFE01`)%>%filter(is.na(b_property_index)==FALSE)%>%select(date,b_property_index)%>%glimpse()
```
